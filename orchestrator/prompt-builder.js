...
class PromptBuilder {
     ...
     */
    buildPrompt(userQuery, selectedAgents) {
        const sections = [];
        // Section 1: Preamble
     ...
     */
    buildPreamble(userQuery, agentCount) {
        const lines = [];
        lines.push('');
     ...
     */
    buildAgentSelectionSummary(selectedAgents) {
        const lines = [];
        lines.push('');
     ...
     */
    buildDetailedAgentInstructions(selectedAgents) {
        const lines = [];
        lines.push('');
     ...
     */
    buildExecutionFramework(selectedAgents) {
        const lines = [];
        lines.push('');
     ...
     */
    buildSafetyConsiderations(selectedAgents) {
        const highRiskAgents = selectedAgents.filter(a => a.agent.risk_level === 'high' || a.agent.risk_level === 'critical');
     ...
     */
    buildSuccessCriteria(selectedAgents) {
        const lines = [];
        lines.push('');
     ...
     */
    exportAgentsAsJson(selectedAgents) {
     ...
     * @returns CSV string
     */
    exportAgentsAsCsv(selectedAgents) {
