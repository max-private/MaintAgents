...
class AgentRouter {
     ...
     */
    selectAgentsForQuery(userQuery, maxAgents = 3) {
     ...
     */
    isTriggerMatch(trigger, queryKeywords) {
        const triggerWords = trigger.toLowerCase().split(/[-\s]+/).filter(w => w.length > 1);
        return triggerWords.every(word => queryKeywords.some(k => k === word || k.includes(word) || word.includes(k)));
    }
    /**
     ...
     */
    isPartialMatch(target, keywords) {
     ...
     */
    getAgentsByKeywordTrigger(triggerKeyword) {
