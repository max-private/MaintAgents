...
> Non-compliant (WRONG): `MyModule.pm` · `**MyModule.pm**` · a plain bullet
> Compliant (CORRECT): [`lib/MyModule.pm:8`](lib/MyModule.pm#L8)
...
**Also affects:** [`path/to/OtherModule.pm:line`](path/to/OtherModule.pm#Lline) — one link per affected file, same format

Rules:
- The heading MUST be a markdown hyperlink `[...](...)` — NOT bold text, NOT plain filename, NOT a separate bullet. Example of non-compliant: `MyModule.pm` or `**MyModule.pm**`. Compliant: [`lib/MyModule.pm:8`](lib/MyModule.pm#L8)
...
- The After block MUST be a complete working replacement — for simple `use` swaps show the full module header; for OO rewrites (e.g. manual bless → Moo) show the complete migrated module including all attribute declarations, method signatures, and constructor
